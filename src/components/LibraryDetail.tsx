...
interface Props {
   ...
   * 검색 목록 옆 칸에 들어갈 때.
   *
   * 투명한 머리글(뒤로 가기) 밑으로 사진을 밀어 넣지 않고, 못 찾았을 때
   * "홈으로" 도 두지 않는다. 옆에 목록이 그대로 있기 때문이다.
   */
  embedded?: boolean;
  /**
   * 사람이 직접 골랐을 때만 "최근 본 도서관" 에 남긴다.
   *
   * 아이패드 검색 화면은 처음 열 때 옆 칸에 첫 도서관을 미리 띄운다. 그것까지
   * 기록하면 검색 탭을 열기만 해도 서울도서관이 늘 "최근 본 곳" 맨 위에 올랐다.
   */
 ...
 *
 * 한 곳에서 두 가지로 쓴다.
 *   - app/library/[id].tsx  QR·홈·즐겨찾기에서 들어오는 전체 화면
 *   - 아이패드 검색 화면     목록 오른쪽 칸
 *
 * 폭이 900 이상이면(가로로 눕힌 아이패드의 전체 화면) 위쪽을 두 단으로 나눈다.
 * 왼쪽에 사진, 오른쪽에 이름·운영 여부·정보. 폰 배치를 그대로 늘리면 사진이
...
export function LibraryDetail({ id, embedded = false, recordRecent = true }: Props) {
  ...
  useEffect(() => {
    if (recordRecent && library?.id) pushRecent(library.id);
  }, [recordRecent, library?.id, pushRecent]);
  ...
  const wide = layout.width >= TWO_COLUMN;
  const gutter = layout.gutter;

  /* ── 조각들. 폰 배치와 두 단 배치가 같은 조각을 다르게 늘어놓는다 ── */
    ...
    <>
      <View style={styles.badgeRow}>
        ...
        {library.categories.map((c) =>
          ...
          CATEGORY_MAP[c] ? <Badge key={c} label={t(`cat.${c}`)} category={c} /> : null
        )}
      </View>
  ...
  /** 지도 앱 버튼. 두 단 배치에서는 오른쪽 단 안에 들어가므로 좌우 여백을 따로 받는다 */
  const mapSection = (inset: number) => (
  ...
  /** 책과 주변 장소. 어느 배치에서든 칸 끝까지 흐르는 가로 줄이다 */
  const shelves = (
    <>
      ...
      {library.coords ? (
        <View style={styles.section}>
          <SectionHeader title={t('lib.nearbyTitle')} subtitle={t('lib.nearbySub')} />
  ...
  /* ── 두 단 배치 ───────────────────────────────────────────── */
  if (wide) {
    const inner = layout.maxContentWidth - gutter * 2;
    const left = Math.round(inner * 0.46);
            ...
            <View
              style={[
                styles.wideTop,
                {
                  ...
                  // 전체 화면일 때는 투명한 머리글(뒤로 가기) 밑으로 들어가지 않게 내린다
                  paddingTop: embedded ? spacing.xl : insets.top + 56,
                  gap: COLUMN_GAP,
                },
              ]}
            >
