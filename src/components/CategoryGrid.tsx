 ...
 * 주제 그리드.
 *
 * 분류가 14개다. 폰에서는 4열(마지막 줄 2개), 태블릿에서는 7열 2줄로 떨어진다.
 * 열 수를 화면 폭에서 받아 오므로 아이패드에서 아이콘 넷이 휑하게
 * 벌어지는 일이 없다.
 */
export function CategoryGrid({ onSelect, selected }: Props) {
  ...
  return (
    <View style={[styles.grid, { paddingHorizontal: layout.gutter - 4 }]}>
      {CATEGORIES.map((cat) => {
        ...
        return (
          ...
          >
            <View
              style={[
                styles.iconBox,
                { width: box, height: box, backgroundColor: palette.bg },
...
const styles = StyleSheet.create({
  ...
  item: {
    // 열 수는 화면 폭에 따라 바뀐다 (useLayout)
    alignItems: 'center',
    gap: 2,
  },
  iconBox: {
    // 크기도 화면 폭에 따라 바뀐다 (useLayout)
